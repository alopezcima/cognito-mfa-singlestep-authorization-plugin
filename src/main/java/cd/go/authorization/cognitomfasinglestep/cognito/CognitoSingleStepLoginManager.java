...
public class CognitoSingleStepLoginManager {
    private final AWSCognitoIdentityProvider cognitoProvider;
    private final String cognitoClientId;

    public CognitoSingleStepLoginManager(Configuration configuration) {
        this.cognitoProvider = configuration.getCognitoIDPProvider();
        this.cognitoClientId = configuration.getClientId();
    }

    public User login(String user, String password, String totp) {
        ...
        try {
            ...
            return new User(cognitoProvider.getUser(userRequest));
